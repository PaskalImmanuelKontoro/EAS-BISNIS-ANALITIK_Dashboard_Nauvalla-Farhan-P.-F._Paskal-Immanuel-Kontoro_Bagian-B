...
server<-function(input,output,session){
  ...
  output$plot1<-renderPlot({
    ggplot(data=data, aes(x= Kabupaten.Kota, y=Jumlah.Dokter)) + 
      ...
      geom_text(aes(label=Jumlah.Dokter))+
      theme_minimal()
  })
